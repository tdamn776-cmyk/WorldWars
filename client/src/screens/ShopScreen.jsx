...
export default function ShopScreen() {
  ...
  const handleBuyChest = (chest) => {
    if (spendBp(chest.cost)) {
      ...
      // Simulate network/opening delay
      setTimeout(() => {
        setOpening(false);
        setReward(result);
        ...
        if (result.type === 'tank') {
          // Find a random tank of that rarity
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 25px', background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <button onClick={() => setScreen('mainMenu')} style={{ fontSize: '18px', fontWeight: 'bold' }}>
          &larr; BACK
        </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center', alignItems: 'flex-start' }}>
        ...
        {CHESTS.map(chest => (
          <div key={chest.id} style={{
            width: '280px', background: 'rgba(25,25,35,0.8)', 
            border: `2px solid ${chest.color}`, borderRadius: '16px',
            padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center',
            boxShadow: `0 0 20px ${chest.glow}`,
            position: 'relative', overflow: 'hidden'
          }}>
            <h2 style={{ color: chest.color, fontFamily: 'var(--font-heading)', margin: '0 0 10px 0', textTransform: 'uppercase' }}>
              {chest.name}
            </h2>
            ...
            <div style={{ width: '100%', background: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '8px', fontSize: '12px', color: '#aaa', marginBottom: '20px' }}>
              Includes:<br/>
              {chest.loot.map((l, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                  <span>{l.type.toUpperCase()} {l.rarity}</span>
                  <span>{Math.round(l.chance * 100)}%</span>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              disabled={bp < chest.cost || opening}
              style={{
                width: '100%', padding: '15px', borderRadius: '8px', fontWeight: 'bold', fontSize: '18px',
                background: bp >= chest.cost ? 'var(--color-accent-gold)' : '#333',
                color: bp >= chest.cost ? '#000' : '#666',
                display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px',
                cursor: bp >= chest.cost && !opening ? 'pointer' : 'not-allowed'
              }}
            >
      ...
      {opening && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
          <div className="animate-pulse" style={{ fontSize: '100px', filter: 'drop-shadow(0 0 50px rgba(255,255,255,0.8))' }}>📦</div>
      ...
      {reward && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
          <div className="animate-slide-up" style={{ textAlign: 'center' }}>
            <h1 style={{ color: 'var(--color-accent-gold)', fontFamily: 'var(--font-display)', marginBottom: '30px', fontSize: '40px' }}>
              REWARD ACQUIRED!
            </h1>
            
            {reward.type === 'bp' && (
              <div style={{ color: 'white', fontSize: '32px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center' }}>
                <img src="/icons/currency-bp.png" alt="BP" style={{ width: '40px' }} />
                +{reward.amount.toLocaleString()} BP
              </div>
            )}

            {reward.type === 'xp' && (
              <div style={{ color: '#3b8bff', fontSize: '32px', fontWeight: 'bold' }}>
            ...
            {reward.type === 'tank' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ fontSize: '24px', color: RARITY[reward.rarity].color, fontWeight: 'bold', marginBottom: '10px' }}>
                  NEW TANK UNLOCKED!
                </div>
                <div style={{ fontSize: '48px', color: 'white', fontFamily: 'var(--font-display)' }}>
                  {reward.tankName?.toUpperCase()}
                </div>
              </div>
            )}
            ...
            <button 
              onClick={closeReward}
              style={{
                marginTop: '50px', padding: '15px 40px', background: 'white', color: 'black', 
